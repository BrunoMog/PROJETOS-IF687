...
print('[second] device=', device, flush=True)
...
results = {}
res, hist = run_ablation_variant(params_without, train_data, test_data, val_data, batch_size=64, num_epochs=1, patience=1, lr=1e-3, save_path=str(MODEL_DIR/'best_without.pth'), device=device, verbose=True, compute_flops=False)
results['without_dropout'] = res

# If there's an existing results file with 'with_dropout', keep it
